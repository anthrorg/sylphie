---
name: rank
description: Rank an incoming question and route it. Escalate architectural, cross-subsystem, cutting-edge, or expensive-to-get-wrong questions to the `mythos` Opus reasoner; answer simple factual/locational questions directly. Use whenever the user asks a question — the question-router hook nudges you here on any message containing "?".
---
...
### TIER 1 — Escalate to `mythos` (Opus reasoner)
...
How to escalate: gather cheap context first (mythos is read-only — point it at the right files/services so it doesn't hunt), then call the `mythos` agent with the question plus those pointers. Relay its answer to Jim; don't re-reason or dilute it.

### Specialist override
If the question sits squarely in one domain specialist's lane, route there instead of `mythos`:
...
`mythos` is the default when no single specialist owns it, or the question spans several.

## After ranking
- Tier 0 → just answer.
- Tier 1 → escalate, then relay.
- If a question implies heavy *implementation* (not just an answer), that's the `opus-agent`'s job, not `mythos` — note it and delegate the build separately.
