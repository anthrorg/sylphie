...
- ✅ **EWC real Fisher + Online EWC** — `training/replay.py` rewritten with empirical Fisher diagonal, Online EWC γ-blend (γ=0.7), λ ramp (200 steps), per-layer Fisher stats logged. `DataBuffer.snapshot_calibration()` added. `POST /cognition/phase-transition` wires `set_reference()` + `compute_fisher()` at runtime. 7 tests, all passing. **Update (2026-07, TK-119..122, pipeline item 20260702-002 / DEC-33 AD-0049-0050):** this consolidation logic was originally reachable ONLY via that manual/guardian-triggered endpoint — nothing in the runtime path ever called it, so "live" overclaimed reality. Fixed end-to-end: the sidecar's `/cognition/cycle` contract bug (flattened `drive_history`, 422 on every call) is fixed, `compute_fisher()`/`set_reference()` ordering corrected, the consolidation logic extracted into a lock-free module-level `_consolidate_phase_boundary()` helper (no lock of its own — `get_weights()`'s existing internal `_weight_lock` acquisition is the only one in the path, deadlock/lock-scope regression-tested), and it is now ALSO invoked automatically from `BootstrapTracker.advance_mode()` at each real phase boundary (shadow→audit→partial→full), not just the manual override. `ConvergenceModel.use_learned` is hard-disabled at both writers (an untrained random head can no longer flip to "learned" mode).
...
#### WS3 — Compounding memory — ✅ CLOSED-WITH-RESIDUALS (2026-06-13)
Turn accumulated knowledge into compounding capability. **Full plan: `wiki/ws3-build-plan.md`.**

**RESULTS (2026-06-13): the knowledge use→reinforce edge is closed and mythos-verified live.** T1–T5 delivered. `recall-retrieval.ts` resolves the grounding node id pre-arbitration (single-hop); `WkgContextService.reinforceFactNode(nodeId, source)` reinforces a recalled-and-used fact node's confidence via the shared ACT-R `computeConfidence()`, capped at the 0.60 ceiling (`max(current, min(0.60, recomputed))` — Std 3, recall-use ≠ guardian confirmation), persisting `retrieval_count`/`last_retrieval_at` to the **correct store per source** (OKG→Neo4j OTHER, WKG→WORLD, no three-graph crossing); `ConfidenceDecayService` now decays from `coalesce(last_retrieval_at, updated_at, created_at)` so used knowledge is durable and unused fades (WORLD lane; OTHER decay deferred — guardian identity facts must not silently fade). Gate rows **C3.1–C3.4** (reinforcement asymmetry, post-decay upward divergence treatment 0.5982 vs control 0.0665, write-recency guard, ceiling) + **C3PROV** (grounding provenance resolves to a real live-Neo4j node — closes the deferred C1 item from line 73) all green; full `yarn gate` 25/0/3, mythos-reproduced 4× byte-stable. **mythos live-smoke drove a real WKG-entity recall conversation turn and watched `retrieval_count` advance 0→4 organically** (the link the gate corpus can't cover). A real `groundingProvenance` plumbing drop at the `cb_speech` boundary was found and fixed.

**Residuals carried forward:** M2 (tensor world-model MAE) baseline is too tight (1-in-4 flaky at ceiling 0.0961) — **pre-existing, WS3 doesn't touch the tensor path**; re-baseline via `gate:update-baseline` or widen tolerance (`learning`/`forge`). Guardian-grade (≥0.60) facts compound as decay-resistance + retrieval_count, not confidence growth (correct by Std 3; doc note added). OTHER-instance decay deferred (stub §2.11). Multi-hop / spreading-activation recall coverage remains Phase 3 (deferred; gated behind the §2.8 WKG leak fix + governor redesign) — WS3 is single-hop only and does not widen the §2.8 leak.

**Thesis (code-confirmed):** memory compounds when *retrieval-and-use reinforces knowledge confidence*. There are two confidence systems and only one compounds: **procedure** confidence has a real ACT-R `reinforced` path (count, `lastRetrievalAt`, graduation — `confidence-updater.service.ts`), but **fact/WKG** confidence only ever *decays* or bumps on re-extraction MERGE — there is **no retrieval-reinforcement**. `confidence-decay.service.ts:21-23` says so verbatim: it uses `updated_at` as a proxy *"since retrieval counts are not yet tracked on nodes."* So a fact recalled 100× decays identically to one never touched. **WS3 closes the use→reinforce edge for knowledge.**

- **Procedure-conflict fix** — ✅ **DONE (2026-06-10, `2e62de6`)**. `fetchExistingTriggerContexts()` now queries Neo4j WORLD for existing ActionProcedure trigger contexts (was returning an empty set → duplicate procedures fragmenting Type 1 confidence). Fails-closed: a WORLD outage DEFERS (bounded at 5 re-enqueues, then dropped loudly) rather than writing a possible duplicate. (Stub inventory §1.3 RESOLVED.)
- **Phase 1 (curve-mover):** T1 grounded retrieval records the grounding node id (extends C1 `groundingProvenance`, pre-arbitration) · T2 Neo4j-persisted fact reinforcement reusing the ACT-R `reinforced` formula, **capped at the 0.60 ceiling (Std 3)** · T3 decay switches from the `updated_at` proxy to real `lastRetrievalAt`/`retrieval_count`.
- **Phase 2 (prove it):** T4 hermetic C3 gate row — recalled-and-used node diverges upward from a never-recalled control, never exceeds 0.60 · T5 grounding provenance verified against live Neo4j (the deferred C1 item from line 73) · T6 retire the provenance-ratio-as-compounding-proxy misconception (it's a *creation* census; read-side retrieval cannot move it).
- **Phase 3 (DEFERRED):** multi-hop / spreading-activation recall coverage — gated behind (A) the §2.8 WKG person-fact leak fix [atlas], (B) depth-governor redesign + (C) priming↔inhibition coupling contract [ashby]. The `spreading_activation.py` engine is **dead code** (stub §2.9), not a turnkey switch; its design is only a reference spec.
...
#### WS5 — Grounding payoff — ✅ CLOSED-WITH-RESIDUALS (2026-06-14)
Perception that visibly changes behavior; multimodal episodic memory she can recall. **Full plan: `wiki/ws5-build-plan.md`; review record: `wiki/reviews/ws5-build-plan/`.**

**RESULTS (2026-06-14): the perception Sylphie already runs is no longer decorative — it moves drives and becomes recallable episodic memory, gate-proven, with the real camera→sidecar leg live-verified (mythos).** Shipped T0–T2 + the surviving gate rows; T3 (face→Type-1 reflex) deferred to WS5.5. The plan went through a **2-round, 14-agent `/review-plan`** (verdict PROCEED-WITH-CHANGES, 0 blocks, CANON cleared) before a line was built — the review caught two structural defects (T0 injecting below the gateway; T3's "cannot leak by construction" being false) and reshaped the plan. **T0** — perception gate-injection seam at the real `PERCEPTION_HOST` HTTP boundary (cassette + inbound `/ws/perception` camera stub + caption-settle barrier + reset endpoints; synthetic frames carry `synthetic:true`, `provenance_type` stays SENSOR — atlas-ruled). **T1** — `Episode.source` required discriminant + `visualContext` with per-sub-field provenance + the `sceneSurprise→input.attention` cycle-ordering refactor (a salient-but-CALM frame now encodes — drive-cold forcing smoke green). **T2** — `queryByContent` recall (overlap-coefficient, per-episode provenance, recall-local ageWeight decay, mood-congruent α=0.20 + ashby rumination circuit-breaker) — NL "did you see a mug earlier?" recalls the `source='perception'` episode, no provenance laundering (smoke green ×2). **T4 gate rows** — P1a (scene-surprise→Curiosity+Anxiety via the real routed-outcome seam), P1b (unknown-person→Social), **P1c (identity-keyed habituation — the curiosity-leak control loop, surprise₁=1.000→surprise₂=0.625 on a fresh trackId, proven non-vacuous)**, P3 (episode stored), P6 (lesion: drives move, no fabricated caption), P4 (recall), P2 (caption-in-prompt). Per-row hermetic isolation makes them **deterministic** (prows smoke 3× all-green). **Full `yarn gate`: 32 pass / 1 fail / 4 skip, 0 cassette misses — non-regression CONFIRMED** (all pre-WS5 checks green; the 1 FAIL is M1 type-ratio, a stale-baseline denominator artifact from the added perception cycles — Type-1 count unchanged — not a regression).

**Honest residuals carried forward:** M1 baseline needs re-anchoring (`gate:update-baseline`) to the WS5 cycle profile; P2 smoke assertion is flaky-but-behaviorally-correct (reads the wrong turn's prompt-capture under queue backlog — needs turn-correlation; the caption demonstrably reaches the real prompt + episode, P4 proves it); real-sidecar leg verified at the HTTP+socket boundary but not with a physical camera/real COCO object (environment-limited); **WS5.5 deferred** — T3 face→Type-1 reflex + face-privacy machinery, T5 world-fact promotion, §2.8 WKG person-fact leak, and consolidation→`visualContext` (stub inventory §2.12 — visual episodes are recall-only, contribute zero to the WKG semantic census). CANON: no violations.
...
3. **WS4** — the presence leap. ✅ CLOSED-WITH-RESIDUALS.
4. **WS3** — compounding memory. ✅ CLOSED-WITH-RESIDUALS (2026-06-13) — closed the knowledge use→reinforce edge (`wiki/ws3-build-plan.md`).
5. **WS5** — grounding payoff. ✅ CLOSED-WITH-RESIDUALS (2026-06-14) — perception moves drives + recallable memory, gate-proven, real-sidecar leg live (`wiki/ws5-build-plan.md`). T3 face-reflex → WS5.5.
