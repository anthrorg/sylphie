...
- **Backbone swaps are versioned destructive migrations, never online drift.** DINOv2-large 1024-D for objects (buildable now — torch/transformers installed), ArcFace 512-D for faces (M0-blocked on `insightface`), per-modality dims (1024 object / 512 face, NOT uniform). EfficientNet is retired LAST behind a grep gate (two consumers).
...
- **Open questions surfaced for Jim, not invented around:** unmeasured CPU latency (DINOv2-large at 15fps may push `/detect` into seconds); Neo4j class-correct-vs-live-wired scope; object-crop retention for re-embedding migration; ByteTrack adopt/forbid (the one Seeing-Clearly item with no home in either other doc); cassette honesty-vs-green default; and the `.passthrough()`→`.strict()` CANON-hardening decision.
...
- **DINOv2-large 1024-D** object embeddings (buildable now); **ArcFace/InsightFace 512-D** face embeddings (M0-blocked); **per-modality dims** (object 1024 / face 512, NOT uniform).
...
**Pre-download weights** so first boot is deterministic: EfficientNet-Lite4 ONNX *(skip if going straight to DINOv2 — it retires this dead path; otherwise verify the actual output dim == 1280)*, DINOv2-large (~1.1GB), ArcFace `buffalo_l`, Moondream2.
...
| **P3** | Backbone swaps as versioned migrations (DINOv2 1024 objects, ArcFace 512 faces, Neo4j persistence) | One-line input-dim change *because* P1 built #0 generically |
...
- **DINOv2-large 1024-D object embeddings (BUILDABLE NOW).** Add `DINOv2EmbeddingExtractor` to `feature_extraction.py` (same protocol, drop-in); modify ONLY the `_extract_track_embedding` swap site keeping the `_embedding_init_failed` gate, double-checked lock, and `cv2.fillPoly` mask-zeroing byte-for-byte; broaden `except` to catch HF download failure (degrade to null, not crash). Migrate `visual_object_embeddings vector(1280)→vector(1024)` (destructive ALTER + re-embed). **Update #0's input-dim const 1280→1024** (the one-line change P1 set up). BONUS: fixes the already-dead Onnx path. *(M / MED — CPU-only inference is slower than EfficientNet; needs a measured latency budget — see §9.)*
...
| `test/gate/perception-path.contract.spec.ts` | **NEW perception interception** | Corrected: there is no cassette to lockstep. The gate never intercepts `:8430` today. This is net-new harness work — add a perception-path interception so the gate can replay a golden `/detect` response; a single source-of-truth `OBJECT_EMBEDDING_DIM` constant gates the vector length so the 1280→1024 migration is a one-line change. |
...
| `test_dinov2_extractor.py` *(ships w/ DINOv2)* | dim==1024; deterministic; **mask-zeroing intact** (masked vs unmasked crop differ; all-background → near-constant). | new `DINOv2EmbeddingExtractor` |
...
New dir `test/fixtures/vision/`: a **synthetic frame generator** (`synth_frames.py`, Pillow — labeled rectangles at known pixel positions, parameterized by frame size for the P2 multi-resolution tests, emits JPEG + ground-truth JSON); **golden vectors** (`golden/` — Xavier first-rows, 768-vec encoder outputs, a DINOv2 1024-vec gated by `embeddingVersion`); a **scripted occlusion/crossing clip** (feeds the tracker test + the future ByteTrack decision); a **multi-angle face set**; **seeded DB fixtures** (`seed_timescale.sql`, `seed_neo4j.cypher` with JSON `bounding_box`); a **mock extractor** (TS `FakeEmbeddingExtractor` / Python `_StubExtractor` returning deterministic unit vectors so Layer-4 runs WITHOUT loading 1.1GB DINOv2). **Runners:** pytest (`pyproject.toml` markers + skip-loud `conftest.py`), jest (extend 3 configs + **add one to `apps/sylphie`**), tsx (E2E). **docker-compose seeding in CI** (`docker-compose.test.yml` brings up TimescaleDB + Neo4j; net-new, no `.github/` exists today).
...
| EfficientNet→**DINOv2 1024** | — | ❌→build (DINOv2) | dim ❌→build | — | **A accept** |
...
```
PASS (post-DINOv2): `embedding_len= 1024`. PASS (pre-DINOv2): `1280`. FAIL: `None` (`_embedding_init_failed` set). A track must be CONFIRMED — POST the same frame 3–4×. `faces= >0` for a face frame.
...
```
PASS: ≥1 node, `provenance_type='SENSOR'`, `confidence=0.40` (≤0.60 ceiling); `vector_dims` = 1280 today / 1024 post-DINOv2.
...
**Test A — Embeddings flow end-to-end (#0 + DINOv2):** POST `mug` then `book` (same desk, same COCO superclass) 4× each. PASS: `tracked_objects[].embedding` non-null, len 1024; intra-instance cosine (same object across angles) > inter-instance (mug vs book) by the target margin; mask-zeroing intact (masked vs unmasked differ). FAIL: embeddings null, or mug≈book collapse.
...
- **Latency:** add per-`/detect` timing; healthy YOLO-only ~30–80ms CPU; with DINOv2-large per confirmed track expect **seconds** on CPU (no CUDA) — if latency > frame interval, latest-wins drops kick in (degradation working, but watch the effective FPS floor).
...
- **Latency is genuinely unmeasured.** torch is CPU-only; DINOv2-large per-confirmed-track at 15fps may push `/detect` into seconds and trigger latest-wins drops (and make the scene-cycle rate bursty/irregular — harder for loop-gain). A real `/perception/detect`-under-load measurement is a mandatory live-smoke prerequisite before committing P3 (and #8/#9/#12). Route the budget to ashby; report nightly (CPU CI would be flaky as a hard gate).
...
| P3.1 | DINOv2-large 1024-D object embeddings + `vector(1280)→1024` migration + #0 const→1024 | [buildable-now] | D1 margin; embeddings flow end-to-end; mask-zeroing intact; cassette/golden 1024 in lockstep |
...
| P5.1 | Retire EfficientNet `OnnxEmbeddingExtractor` (keep `DominantColorExtractor`) | [buildable-now] | grep ZERO `OnnxEmbeddingExtractor`/`efficientnet`; `/detect` still 1024/512 |
| ACC.A | **A acceptance** (`accept-A.e2e.ts`) — embeddings flow end-to-end | [blocked-on-m0] | non-null 1024; intra>inter margin; mask-zeroing; **mythos live-smoke** |
