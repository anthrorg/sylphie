...
- **Deferred-with-reason (NOT in Wave 1):** §2.8 privacy leak → WS5.5 (atlas design); ~~theater-prohibition-*real-validation* → needs sentiment-vs-drive design~~ **✅ DONE 2026-06-14** (deterministic lexical affect scorer `apps/sylphie/src/services/theater-affect-scorer.ts`; CHEERFUL/DISTRESS_THEATER classes; verdict threads to `reportBasicOutcome` → `theaterValidated` → drive-engine zero-reinforcement; THEATER_PROHIBITED audit emit; 7/7 self-test, apps/sylphie build green); sensory-logger standin → load-bearing (sole `[sensory]` telemetry producer).
- **Carried flags:** Std-3 schema CHECK untested vs live DB (→ fold into Wave 2 live verification); ~~perception stub-inventory/cv-framework docs still list the deleted `/perception/stream` endpoints~~ **✅ docs pass done 2026-06-14** (stub §4.1 + cv-framework table updated; deletion verified absent in `main.py`).
