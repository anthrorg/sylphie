...
export class DecisionMakingService implements IDecisionMakingService, OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.cycleGuard.register(
      ...
      () => this.tickEngine.isSelfTickInFlight(),
    );

    // EP7-A: Wire the tick engine's processInput callback before starting.
    this.tickEngine.wire({
      processInput: (frame) => this.processInput(frame),
    });

    this.tickEngine.startTickLoop();
  }

  async onModuleDestroy(): Promise<void> {
    this.tickEngine.stopTickLoop();
  ...
  // EP7-A (TK-31): Extracted to DecisionTickEngineService.
  // startTickLoop / stopTickLoop / onTick / pickResearchTarget all live in
  // tick-engine/decision-tick-engine.service.ts. DecisionMakingService wires
  // the processInput callback via tickEngine.wire() in onModuleInit().
  // ---------------------------------------------------------------------------
