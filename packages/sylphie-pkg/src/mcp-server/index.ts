 ...
 *
 * Registers 6 tools that let Claude Code agents query codebase structure
 ...
 *   getLogContext      — query log summary nodes
 *
 * Usage:
...
const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        scope: {
          type: 'string',
          description: 'Service, module, or function name to find constraints for. Examples: "observatory", "executor", "neo4j".',
        },
      },
      required: ['scope'],
    },
  },
  {
    name: 'getLogContext',
    description:
      'Query log summary nodes stored in the PKG. ' +
      'Returns log descriptions, severity, timestamps, and the functions that emit them. ' +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Optional text to search in log descriptions.',
        },
        service: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        fileFilter: {
          type: 'string',
          description: 'Optional partial file path to narrow the search (e.g., "orchestrator", "maintenance-engine").',
        },
        maxResults: {
          type: 'number',
  ...
  try {
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `Error executing ${name}: ${message}\n\nThis may indicate the sylphie-pkg Neo4j instance is not running on bolt://localhost:7689.`,
        },
      ],
      isError: true,
    };
