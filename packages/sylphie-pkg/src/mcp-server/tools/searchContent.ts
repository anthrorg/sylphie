 ...
 *
 * Searches CodeBlock.bodyText using case-insensitive regex, then returns
 * the parent Function/Type metadata for each match. Optionally filters
 * by file path substring.
 */
export async function handleSearchContent(input: SearchContentInput): Promise<string> {
  ...
  // Build case-insensitive regex for Neo4j
  ...
  // Fallback: also search old-style bodyText on Function nodes (pre-migration)
    ...
    if (bodyText) {
      ...
      for (let i = 0; i < bodyLines.length; i++) {
        if (bodyLines[i]!.toLowerCase().includes(patternLower)) {
          ...
          if (matchingLines.length >= 5) break; // Cap at 5 matching lines per function
        }
      }
