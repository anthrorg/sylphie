 ...
 *
 * Searches for Constraint nodes reachable from Service, Module, or Function
 * nodes whose name contains the scope string. Also returns constraints
 * stored directly on Constraint nodes whose own name/area matches.
 *
 * @param input - Tool input with the scope string to search.
 * @returns Formatted constraint list sorted by severity.
 */
export async function handleGetConstraints(input: GetConstraintsInput): Promise<string> {
  ...
  // Deduplicate by description (same constraint may appear via multiple owners)
  ...
  if (allConstraints.length === 0) {
    return (
      ...
      `Check the CANON (wiki/CANON.md) for architectural rules that may not yet be loaded into the PKG.`
    );
  }
  ...
  lines.push('='.repeat(60));
  lines.push(`Always consult wiki/CANON.md for the authoritative source of architectural rules.`);
