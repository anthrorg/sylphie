...
export async function handleGetModuleContext(input: GetModuleContextInput): Promise<string> {
  ...
  // "executor engine" → matches anything containing "executor" OR "engine"
  ...
  // All strategies run in one query via UNION to avoid fallback gaps
  ...
  // Collect module file paths for querying (more reliable than name matching)
