...
export interface GetRecentChangesInput {
  query: string;
  since?: string; // ISO date string, e.g. "2026-02-01". Defaults to 30 days ago.
}

/**
 * Handle the getRecentChanges tool call.
 *
 * Matches Change nodes by description/prDescription text, then finds which
 * functions and types were modified in those changes.
 *
 * @param input - Tool input with query string and optional since date.
 * @returns Formatted change history suitable for LLM consumption.
 */
export async function handleGetRecentChanges(input: GetRecentChangesInput): Promise<string> {
  ...
  // Build lookup maps from changeId -> affected nodes
  ...
  for (const r of changeRecords) {
    ...
    const area = r.get('area') as string | null;
    const desc = r.get('description') as string | null;
    const prDesc = r.get('prDescription') as string | null;
    ...
    if (prDesc && prDesc !== desc) lines.push(`  ${prDesc}`);
...
/**
 * Returns an ISO date string 30 days before today.
 */
