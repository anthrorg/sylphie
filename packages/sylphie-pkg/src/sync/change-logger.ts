 ...
 * modified files. This gives agents the ability to answer "what changed in
 * this commit?" and "which functions have been touched most recently?".
 *
 * Change nodes are lightweight: they store the commit hash, author, date,
 * and commit message. They do not store diffs.
 */
...
/**
 * Read commit metadata from git log for a given commit hash.
 */
...
/**
 * Record a Change node for the given commit and link it to all Function/Type
 * nodes in the changed files.
 *
 * This is a best-effort operation: if a Function or Type node does not yet
 * exist for a changed file (e.g., the file was added in this commit and
 * parsing partially failed), that link is silently skipped.
 *
 * @param changedFilePaths - Absolute paths to files changed in this commit.
 * @param commitHash - The commit hash to log. Defaults to HEAD.
 */
export async function logChange(
  ...
  try {
    // Create or merge the Change node
    await tx.run(
      `
    ...
    // Link CHANGED_IN edges from Function nodes in the changed files
    await tx.run(
      `
      MATCH (f:Function)
    ...
    // Link CHANGED_IN edges from Type nodes in the changed files
    await tx.run(
      `
      MATCH (t:Type)
    ...
    // Also link Module nodes (the file-level nodes)
    await tx.run(
      `
      MATCH (m:Module)
...
/**
 * Get the most recent Change nodes from the PKG.
 * Useful for status reporting.
 *
 * @param limit - Maximum number of Change nodes to return. Defaults to 10.
 */
